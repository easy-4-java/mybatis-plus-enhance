...
当前分支属于 `3.0.x`，关键依赖固定为：

| 组件 | 版本或系列 | 选择原因 |
|---|---|---|
| Java | 21 | `3.0.x` 的最低编译与运行环境 |
| Spring Framework | 6.2.11 | 明确支持 JDK 21，并保持稳定的 Jakarta 技术栈 |
...
`3.0.x` 不得降级到 Spring 5、`javax.annotation` 或 JSqlParser 4.9；新增实现可以使用
JDK 21 语言特性和标准库 API，但应保持公共 API 语义与其他版本线一致。
...
`3.0.x` 发布前还应检查产物 Class 文件主版本为 `65`，并确认依赖树中不存在 Spring 5、
